...
class BookDetailSidebarTests(TestCase):
    ...
    def test_book_sidebar_contains_title(self):
        ...
        """
        response = self.client.get(reverse('library:book', args=[self.book_single_author.id]))
    ...
    def test_book_sidebar_contains_author_links(self):
        """
        ...
        author_url = reverse('library:author', args=[self.author1.id])
    ...
    def test_book_sidebar_multiple_authors(self):
        """
        Verify that multiple authors are displayed if present.
        """
        response = self.client.get(reverse('library:book', args=[self.book_multiple_authors.id]))
        self.assertEqual(response.status_code, 200)
        
        author1_url = reverse('library:author', args=[self.author1.id])
        author2_url = reverse('library:author', args=[self.author2.id])
