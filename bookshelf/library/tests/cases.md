...
10. **test_author_list_view_filter_summary_parameterized** *(parameterized: matched_prefix, matched_regex, unmatched_prefix, unmatched_regex)*: Verifies the summary badge names the node when the tree resolves it (`Last name: l`, `Last name: other`) and shows the raw filter value when it does not (`Prefix: XYZ`, `Last name matches: ^zzz`).
